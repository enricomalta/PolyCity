...
import type { EconomicModel, PoliticalIdeology } from "@/types/city"
...
export function CityCreationModal({ onCreated }: { onCreated: () => void }) {
  ...
  const [economicModel, setEconomicModel] = useState<EconomicModel>("SOCIAL_MARKET")
  const [ideology, setIdeology] = useState<PoliticalIdeology>("SOCIAL_DEMOCRACY")

  const ECONOMIC_MODELS: { value: EconomicModel; label: string; description: string }[] = [
    { value: "SANDBOX", label: "Sandbox", description: "Liberdade total para definir o governo." },
    { value: "SOCIAL_MARKET", label: "Economia social de mercado", description: "Mercado com proteção social e serviços públicos." },
    { value: "FREE_MARKET", label: "Livre mercado", description: "Baixa intervenção e impostos mais leves." },
    { value: "PLANNED_ECONOMY", label: "Economia planejada", description: "O Estado coordena preços e setores estratégicos." },
    { value: "WELFARE_STATE", label: "Estado de bem-estar", description: "Alta proteção social financiada por impostos." },
  ]
  ...
  const create = async () => {
    ...
    setError(null)
    try {
      await apiGameService.createCity("me", { name: name.trim(), economicModel, ideology })
      onCreated()
    } catch {
  ...
  return (
    <main className="flex min-h-svh items-center justify-center bg-background px-4 py-8 text-foreground">
      <section className="w-full max-w-3xl rounded-3xl border border-border bg-card p-6 shadow-2xl sm:p-8">
        ...
        <div className="mt-6"><div className="flex items-center gap-2 text-sm font-semibold"><Landmark className="size-4 text-primary" />Modelo econômico</div><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{ECONOMIC_MODELS.map((item) => <button key={item.value} type="button" onClick={() => setEconomicModel(item.value)} className={`min-h-28 rounded-2xl border p-4 text-left transition ${economicModel === item.value ? "border-primary bg-primary/10 shadow-lg shadow-primary/10" : "border-border bg-background hover:bg-secondary"}`}><div className="font-semibold">{item.label}</div><div className="mt-1 text-xs leading-relaxed text-muted-foreground">{item.description}</div></button>)}</div></div>
